            ...
            if (val.length == 0) {
                ...
                fs.access(pathfile, function (err) {
                    if (!err) {
                        ...
                        const controller_name = capitalize.lowercase(val);
 ...
 * 
 * For example
 * If you want to get access ${capitalize.lowercase(val)}/${capitalize.lowercase(val)}_get with GET then simply go for ${capitalize.lowercase(val)}/${capitalize.lowercase(val)}.
 * If you used POST method it will access ${capitalize.lowercase(val)}_post.
 * 
 * In case you want to define your own then use methodname_requestmethod i.e ${capitalize.lowercase(val)}_career_post
 * 
 * Default is index so you don't need to define the function name in your uri just change the
 * request method name.
 * 
 * In below case GET example.com/${capitalize.lowercase(val)}/1 it will call index_get.  
 * Also if PUT example.com/${capitalize.lowercase(val)}/1 it will call index_put.
 * 
 */
