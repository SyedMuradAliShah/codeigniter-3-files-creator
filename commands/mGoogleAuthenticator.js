...
module.exports = function (vscode, fs, path, pathdir) {
    vscode.window.showInputBox({
        prompt: "Enter name of google auth library file",
        placeHolder: "Enter name of google auth library file"
    }).then(function (val) {
        if (val.length == 0) {
...
class ${capitalize.capitalize(val)}
{
     ...
     *      $this->load->library('${capitalize.lowercase(val)}'); //Calling this library
     * 
     ...
     *      $secretCode = $this->${capitalize.lowercase(val)}->createSecret(); 
     * 
     ...
     *    to image only.
     * 
     *      $this->${capitalize.lowercase(val)}->generateQRCodeImage($qrCodeUrl);
     */
